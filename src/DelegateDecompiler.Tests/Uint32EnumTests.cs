...
{
    [TestFixture]
    public class Uint32EnumTests : DecompilerTestsBase
    {
        ...
        {
            Expression<Func<TestEnum, bool>> expected = x => Int8Method((uint) x);
            Func<TestEnum, bool> compiled = x => Int8Method((uint) x);
            Test(compiled, expected);
        }
        ...
        private static bool Int8Method(uint p0)
        {
            throw new NotImplementedException();
        }
